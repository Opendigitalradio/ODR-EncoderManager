...
if __name__ == '__main__':
	...
	while True:
		...
		try:
			admin_ip = os.popen('ip addr show eth0').read().split("inet ")[1].split("/")[0]
		except:
			admin_ip = False
			
		try:
			lan_ip = os.popen('ip addr show eth1').read().split("inet ")[1].split("/")[0]
		except:
			lan_ip = False
			
		try:
			wan_ip = os.popen('ip addr show eth2').read().split("inet ")[1].split("/")[0]
		except:
			wan_ip = False
		...
		if admin_ip:
			lcd.clear_screen()
			lcd.write( 'DAB+ Encoder ADM')
			lcd.position( 2, 1 )
			lcd.write( admin_ip )
			time.sleep( 2 )
		if lan_ip:
			lcd.clear_screen()
			lcd.write( 'DAB+ Encoder LAN')
			lcd.position( 2, 1 )
			lcd.write( lan_ip )
			time.sleep( 2 )
		if wan_ip:
			lcd.clear_screen()
			lcd.write( 'DAB+ Encoder WAN')
			lcd.position( 2, 1 )
			lcd.write( wan_ip )
			time.sleep( 2 )
		
		with open(config.config['odr']['padenc']['dls_fifo_file'], 'r') as file:
			for line in file:
				dls = line.strip()
